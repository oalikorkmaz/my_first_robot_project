...
class PID(object):
    ...
    def __call__(self, input_, dt=None):
        ...
        if(self.differential_on_measurement == False):
            #-------------derivate yeni hesap----------------
            self._derivative = d_error / dt
            _fCut = 50 # Kesim frekansını buraya yazın (sistem 20hz olduğu için kesimi 50hz yaptık)
            RC = 1 / (2 * math.pi * _fCut)

            self._derivative = self._last_derivative  + ((dt / (RC + dt)) * (self._derivative - self._last_derivative))
            self._last_derivative = self._derivative

            new_derivative = self.Kd * self._derivative
            #-------------------------------------------------
        else:
            new_derivative = self._derivative
        
        # Compute final output
        output = self._proportional + self._integral + new_derivative
...
"""
print("hi")
pid = PID(5 , 0.1, 1, sample_time=0.05) #0.05 second to 20hz
pid.set_auto_mode(True)
pid.output_limits = (-100, 100)
pid.setpoint = 50
output = pid.__call__(55) 
print(output)
"""